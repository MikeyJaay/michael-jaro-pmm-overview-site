...
const CompanyLogos = () => (
  <section className="border-y border-border/40 bg-secondary/30 py-8">
    ...
    <div className="md:hidden overflow-hidden [mask-image:linear-gradient(to_right,transparent,black_12%,black_88%,transparent)]">
      <div className="flex w-max animate-marquee items-center gap-x-4 sm:gap-x-6 pointer-events-none">
        {[...logos, ...logos].map((logo, i) => (
          <div key={i} className="flex w-[120px] sm:w-[140px] shrink-0 items-center justify-center">
            <img
              src={logo.src}
              alt={logo.name}
              loading="eager"
