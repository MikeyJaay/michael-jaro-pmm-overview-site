    ...
    <section className="border-t border-border/50 py-20">
      <div className="container mx-auto px-6">
        <div className="mb-10 text-center">
          <p className="mb-2 text-sm font-medium uppercase tracking-widest text-primary">Explore</p>
        ...
        <div className="mx-auto grid max-w-4xl gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {workLinks.map((item, i) => (
            ...
            >
              <Link
                to={item.path}
                className="group flex flex-col items-center rounded-2xl border border-border/50 bg-card p-6 text-center transition-shadow duration-300 hover:shadow-md hover:shadow-primary/[0.06]"
              >
                <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-xl bg-primary/10 text-primary transition-colors group-hover:bg-primary/20">
                  <item.icon size={22} />
                </div>
                <h3 className="mb-1.5 font-display text-sm font-semibold text-foreground">{item.label}</h3>
